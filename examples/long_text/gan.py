...
def _main(_):
    ...
    start_tokens = tf.ones_like(data_batch['source_length']) * boa_id
    ...
    clas_embedder = tx.modules.WordEmbedder(vocab_size=vocab.size,
                                            hparams=args.word_embedding_hparams)
    ...
    for idx, hole in enumerate(answer_packs):
        ...
        _, ecdr_states = encoder(
            enc_input_embedded,
            sequence_length=cur_template_pack["template_lengths"])
        ...
        loss_d_clas = tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.to_float(tf.ones_like(data_batch['source_length'])), logits=clas_logits)
        ...
        loss_g_clas = tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.to_float(tf.zeros_like(data_batch['source_length'])), logits=soft_logits)
    ...
    for idx, hole in enumerate(answer_packs):
        ...
        _, ecdr_states = encoder(
            enc_input_embedded,
            sequence_length=cur_test_pack["template_lengths"])
